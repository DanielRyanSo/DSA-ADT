...
/* arrayStack.h interface implementation */
...
/* Guide-question helpers used from main */

/* 2. Add a container to the next available stack */
...
/* 3. Remove a container from the latest (highest index non-empty) stack */
int removeContainer(Stack stacks[NUM_STACKS]) {
    ...
    printf("All stacks are empty. Nothing to remove.\n");
    return -1;
}

/* 4. Check if a container exists in any stack */
...
int main(void) {
    /* 1. Create and initialize the three stacks (using an array) */
